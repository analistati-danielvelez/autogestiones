...
const getPriceByQuantity = (priceStructure: PriceStructure, quantity: number): {
  ...
  // Si tiene precio fijo (como servicios de mascotas)
  if (priceStructure.fixed) {
    return {
      pricePerUnit: priceStructure.fixed,
      isFixed: true // Solo true para precios fijos por unidad
    };
  }
  
  // Para precios escalonados, retornar el precio exacto para esa cantidad
