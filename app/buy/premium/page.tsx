...
export default function BuyPremiumPage() {
  ...
  const calculateTotal = () => {
    ...
    const costoAsistenciasPersonas = validHumanAssistance.reduce((acc, curr) => {
      ...
      // Si es precio fijo, cada persona paga el precio
      if (isFixed) {
        return acc + (pricePerUnit * totalCount);
      }
      
      // Si no es fijo, el precio es el total para esa cantidad de personas
  ...
  const navigateStep = (step: number | 'next' | 'prev') => {
    if (step === 'next') {
      ...
      if (currentStep === 4 && selectedPetAssistance.length > 0 && pets.length === 0) {
