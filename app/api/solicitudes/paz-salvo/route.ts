...
function generarHtmlSolicitudPazSalvo(datos: {
  nombre: string;
                ...
                <tr>
                  <td style="padding:34px 38px 30px; text-align:center;">
                    ...
                    <p style="margin:18px 0 0; color:#4b5563; font-size:15px; line-height:1.7;">
                      ...
                      <strong style="color:#002869;">Paz y Salvo</strong>.
                    </p>
...
async function enviarCorreoConfirmacion(datos: {
  ...
  await transporter.sendMail({
    ...
    subject: "Solicitud de Paz y Salvo registrada",
    html: generarHtmlSolicitudPazSalvo({
      nombre: datos.nombre,
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p>
          Se recibió una solicitud de <strong>Paz y Salvo</strong> con archivo adjunto.
        </p>
...
const archivoAdjunto =
  ...
  const productosTexto = planesSolicitud
    ...
    await registrarSolicitudNivel2Certificados({
      ...
      tipo: "Paz y Salvo",
      certificado:
        esSolicitudEmpresarial && tieneAdjunto === "SI"
          ? `Paz y Salvo empresarial con adjunto ${archivoAdjunto?.name || ""}`
          : esSolicitudEmpresarial
            ? "Paz y Salvo empresarial"
            : tieneAdjunto === "SI"
              ? `Paz y Salvo titular con adjunto ${archivoAdjunto?.name || ""}`
              : "Paz y Salvo titular",
    });
    ...
    console.error("Error registrando solicitud de Paz y Salvo:", error);

    return NextResponse.json(
      {
        ok: false,
        message: "No fue posible registrar la solicitud de Paz y Salvo en este momento.",
      },
      { status: 500 }
    );
  }
