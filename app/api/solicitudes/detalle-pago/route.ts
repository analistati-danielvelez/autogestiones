...
function validarRangoMaximoDosAnios(fechaInicio: string, fechaFin: string) {
...
async function registrarSolicitudNivel2DetallePagos(datos: {
...
function generarHtmlSolicitudDetallePago(datos: {
  nombre: string;
                ...
                <tr>
                  <td style="padding:34px 38px 30px; text-align:center;">
                    ...
                    <p style="margin:18px 0 0; color:#4b5563; font-size:15px; line-height:1.7;">
                      ...
                      <strong style="color:#002869;">Detalle de pago</strong>.
                    </p>
                    ...
                    <table role="presentation" width="100%" cellspacing="0" cellpadding="0" style="margin-top:24px; background:#f5fafd; border:1px solid #d8edf8; border-radius:12px;">
                      <tr>
                        <td style="padding:18px 22px; text-align:left; color:#374151; font-size:14px; line-height:1.7;">
                          ...
                          <strong style="color:#002869;">Rango solicitado:</strong> ${datos.fechaInicio} al ${datos.fechaFin}
                        </td>
                      </tr>
                    </table>
...
async function enviarCorreoConfirmacion(datos: {
  ...
  await transporter.sendMail({
    ...
    subject: "Solicitud de detalle de pago registrada",
    html: generarHtmlSolicitudDetallePago({
      nombre: datos.nombre,
...
async function enviarCorreoAreaDetallePago(datos: {
    ...
    await transporter.sendMail({
      ...
      html: `
        ...
        <p>
          Se recibió una solicitud de <strong>detalle de pago</strong> con archivo adjunto.
        </p>
  
        <p>
          <strong>Nombre:</strong> ${datos.nombre}<br />
          <strong>Identificación:</strong> ${datos.identificacion}<br />
          <strong>Contrato(s):</strong> ${datos.contratosTexto}<br />
          <strong>Producto(s):</strong> ${datos.productosTexto}<br />
          <strong>Rango solicitado:</strong> ${datos.fechaInicio} al ${datos.fechaFin}
        </p>
  
        <p>
          Se adjunta el documento cargado por el usuario.
        </p>
      `,
      attachments: [
        {
          filename: datos.archivo.filename,
          content: datos.archivo.content,
...
const archivoAdjuntoRaw = formData.get("archivoAdjunto");

const archivoAdjunto =
  archivoAdjuntoRaw &&
  typeof archivoAdjuntoRaw === "object" &&
  "arrayBuffer" in archivoAdjuntoRaw &&
  "size" in archivoAdjuntoRaw &&
  Number((archivoAdjuntoRaw as { size: number }).size) > 0
    ? (archivoAdjuntoRaw as {
        name?: string;
        type?: string;
        size: number;
    ...
    const tieneAdjunto = archivoAdjunto ? "SI" : "NO";
    ...
    if (archivoAdjunto) {
      const nombreArchivo = (archivoAdjunto.name || "").toLowerCase();
    
      const extensionValida = EXTENSIONES_ARCHIVO_PERMITIDAS.some((extension) =>
        nombreArchivo.endsWith(extension)
      );
    
      const tipoValido = TIPOS_ARCHIVO_PERMITIDOS.includes(archivoAdjunto.type || "");
      ...
      if (archivoAdjunto.size > TAMANO_MAXIMO_ADJUNTO_BYTES) {
        return NextResponse.json(
          {
            ok: false,
            message: `El archivo adjunto no puede superar los ${TAMANO_MAXIMO_ADJUNTO_MB} MB.`,
          },
          { status: 400 }
        );
      }
    ...
    if (!fechaInicio || !fechaFin) {
      return NextResponse.json(
        { ok: false, message: "Debe ingresar el rango de fechas solicitado." },
        { status: 400 }
      );
    }
    ...
    if (!validarRangoMaximoDosAnios(fechaInicio, fechaFin)) {
      return NextResponse.json(
        {
          ok: false,
          message:
  ...
  const productosTexto = planesSolicitud
      ...
      const datosDoc = JSON.stringify([
        {
          ...
          tieneAdjunto,
          nombreAdjunto: archivoAdjunto?.name || "",
        },
      ]);

      let archivoParaCorreo: ArchivoAdjuntoCorreo | null = null;

        if (archivoAdjunto) {
        const archivoBuffer = Buffer.from(await archivoAdjunto.arrayBuffer());

        archivoParaCorreo = {
            filename: archivoAdjunto.name || `adjunto-${codigoSolicitud}`,
            content: archivoBuffer,
            contentType: archivoAdjunto.type || undefined,
        };
        }
    ...
    await registrarSolicitudEnSheets({
      ...
      tipoDoc: "Detalle de pago",
      quienNecesitaDoc: "Titular",
      dirigidoADoc: "No aplica",
      datosDoc,
    });

    await registrarSolicitudNivel2DetallePagos({
      fechaSolicitud: obtenerFechaRegistroTexto(),
      cedula: datosTitular.identificacion,
      contrato: contratosTexto,
      nombre: datosTitular.nombre,
      tipoPlan: productosTexto,
      solicitud: esSolicitudEmpresarial
        ? tieneAdjunto === "SI"
          ? `Detalle de pago empresarial del ${fechaInicio} al ${fechaFin}. Usuario adjuntó soporte: ${archivoAdjunto?.name || "archivo adjunto"}`
          : `Detalle de pago empresarial del ${fechaInicio} al ${fechaFin}. Sin soporte adjunto`
        : tieneAdjunto === "SI"
          ? `Detalle de pago del ${fechaInicio} al ${fechaFin}. Usuario adjuntó soporte: ${archivoAdjunto?.name || "archivo adjunto"}`
          : `Detalle de pago del ${fechaInicio} al ${fechaFin}. Sin soporte adjunto`,
      datosEnvio: `${datosTitular.email} / ${codigoSolicitud}`,
    });
    ...
    return NextResponse.json(
      {
        ok: true,
        message:
          "La solicitud fue enviada. La información llegará al correo relacionado a la cédula registrada.",
        codigoSolicitud,
      },
      { status: 200 }
    );
  } catch (error) {
    console.error("Error registrando solicitud de detalle de pago:", error);
